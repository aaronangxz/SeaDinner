...
func TestCheckMute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "Daily reminder notifications are *ON*. Do you want to turn it OFF?",
		},
		{
			name: "HappyCase-Mute",
			args: args{u2.GetUserId()},
			want: "Daily reminder notifications are *OFF*. Do you want to turn it ON?",
		},
		{
			name: "NotExist",
			args: args{12345},
...
func TestUpdateMute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:     "Successfully updated your preference.",
			want1:    true,
